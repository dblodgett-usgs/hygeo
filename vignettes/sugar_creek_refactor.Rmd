...
nhd_crosswalk <- dplyr::mutate(nhd_crosswalk, outlet_COMID = as.integer(COMID)) %>%
  left_join(network_order, by = c("outlet_COMID" = "COMID")) %>%
  group_by(local_id) %>%
  filter(Hydroseq == min(Hydroseq)) %>%
  select(-Hydroseq) %>% ungroup()

nhd_crosswalk <- setNames(lapply(seq_len(nrow(nhd_crosswalk)), function(x, df) {
  out <- setNames(as.list(df[x, 2:ncol(df)]), names(df)[2:ncol(df)])  
  out[!is.na(out)]
}, df = nhd_crosswalk), nhd_crosswalk$local_id)
...
jsonlite::write_json(nhd_crosswalk, file.path(out_path, "crosswalk.json"), pretty = TRUE, auto_unbox = TRUE)
